...
export function renderFlashcards(app) {
  ...
  if (!s.deckId) {
    const decksHtml = decksRaw.map((dk) => {
      ...
      return `<button data-dos-cardlink data-act="${app.act(() => app.setState({ deckId: dk.id, cardFlipped: false }))}" style="text-align:left;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:var(--pad-card);cursor:pointer;color:var(--ink);display:flex;flex-direction:column;gap:8px">
        ...
        <div style="font-family:var(--serif);font-size:20px;font-weight:600;margin-top:4px">${esc(dk.title)}</div>
        <div style="font-size:13px;color:var(--ink2)">${esc(dk.sub)}</div>
        <div class="dos-progress" style="height:6px;margin-top:8px"><div style="width:${pct}%"></div></div>
    ...
    return `<div class="dos-page">
      <div><h1 class="dos-h1">Flashcards</h1><p style="color:var(--ink2);font-size:15px;margin-top:8px">Seven decks from the book’s own one-liners. Again → next session · Hard → 1 day · Good → 3 days · Easy → 7 days.</p></div>
  ...
  if (dueCards.length === 0) {
    body = `<div style="text-align:center;padding:48px 24px;background:var(--surface);border:1px solid var(--border);border-radius:8px;width:100%">
    ...
    body = `<button class="dos-flip-card" data-act="${app.act(() => app.setState({ cardFlipped: !s.cardFlipped }))}">
      <div>
        <div class="dos-eyebrow" style="margin-bottom:16px">${s.cardFlipped ? 'Answer' : 'Prompt'}</div>
  ...
  return `<div style="max-width:560px;margin:0 auto;display:flex;flex-direction:column;gap:24px;align-items:center">
