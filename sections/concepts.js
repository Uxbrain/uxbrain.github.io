...
function renderUnifiedResults(app, q) {
  ...
  const topicsHtml = topicHits.map((id) => `<button data-dos-row data-act="${app.act(() => app.openTopic(id))}" style="display:flex;flex-direction:column;gap:4px;width:100%;padding:16px 16px;border:none;border-radius:8px;cursor:pointer;text-align:left;color:var(--ink)">
  ...
  const glossHtml = glossHits.map((g) => {
    ...
    return `<button data-dos-row data-act="${app.act(() => { if (hasLink) app.openTopic(g.linkTopic); })}" style="display:flex;flex-direction:column;gap:4px;width:100%;padding:16px 16px;border:none;border-radius:8px;cursor:${hasLink ? 'pointer' : 'default'};text-align:left;color:var(--ink)">
      <span style="font-size:15px;font-weight:600">${esc(g.term)}${hasLink ? ' →' : ''}</span>
      <span style="font-size:13px;color:var(--ink2)">${esc(g.def)}</span>
    </button>`;
  }).join('');
  const conceptsHtml = conceptHits.slice(0, 24).map((c) => `<span style="display:inline-flex;align-items:center;gap:8px;height:30px;padding:0 16px;border-radius:16px;border:1px solid var(--border);background:var(--surface-alt);color:var(--ink2);font-size:13px;margin:4px">${esc(c.name)} <span style="color:var(--ink2)">· ${esc(c.cat)}</span></span>`).join('');

  const nothing = !topicHits.length && !glossHits.length && !conceptHits.length;
  return `<div style="display:flex;flex-direction:column;gap:16px">
    ${topicHits.length ? `<div><div class="dos-eyebrow" style="margin-bottom:8px">Chapters &amp; topics</div><div style="background:var(--surface);border:1px solid var(--border);border-radius:8px;overflow:hidden">${topicsHtml}</div></div>` : ''}
    ${glossHits.length ? `<div><div class="dos-eyebrow" style="margin-bottom:8px">Glossary</div><div style="background:var(--surface);border:1px solid var(--border);border-radius:8px;overflow:hidden">${glossHtml}</div></div>` : ''}
    ${conceptHits.length ? `<div><div class="dos-eyebrow" style="margin-bottom:8px">Concept index (${conceptHits.length} match${conceptHits.length === 1 ? '' : 'es'})</div><div style="line-height:2.4">${conceptsHtml}</div></div>` : ''}
...
function renderDayWiseBrowse(app) {
  ...
  const rows = groups.map((g) => {
    ...
    const dayChips = g.days.map((d) => {
      ...
      return `<button style="height:30px;padding:0 8px;border-radius:16px;border:1px solid ${complete ? 'var(--success)' : 'var(--border)'};background:${complete ? 'var(--mint)' : 'var(--surface-alt)'};color:${complete ? 'var(--success)' : 'var(--ink2)'};font-size:12.5px;cursor:pointer" data-act="${app.act(() => app.nav({ sec: 'library', book: g.n, topic: null }))}">Day ${d.n}${complete ? ' ✓' : ''}</button>`;
    }).join('');
    return `<div class="dos-card" style="padding:16px 16px">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:8px">${dayChips}</div>
    </div>`;
  }).join('');
  return `<div style="display:flex;flex-direction:column;gap:16px">
...
function renderConceptIndex(app) {
  ...
  const categoriesHtml = CB.CONCEPT_CATEGORIES.map((cat, ci) => {
    ...
    const chips = isOpen ? `<div style="display:flex;flex-wrap:wrap;gap:8px;padding:0 24px 16px">${cat.concepts.map((name) => {
      ...
      return `<button style="height:32px;padding:0 16px;border-radius:16px;border:1px solid ${border};background:${bg};color:${color};font-size:13px;cursor:${tid ? 'pointer' : 'default'}" data-act="${app.act(() => { if (tid) app.openTopic(tid); })}">${esc(name)}${tid ? ' →' : ''}</button>`;
    }).join('')}</div>` : '';
    return `<div class="dos-card" style="padding:0;overflow:hidden">
      <button style="width:100%;display:flex;align-items:center;gap:16px;padding:16px 24px;background:transparent;border:none;cursor:pointer;text-align:left" data-act="${app.act(() => app.setState({ conceptOpen: Object.assign({}, s.conceptOpen, { [cat.id]: !isOpen }) }))}">
...
function renderGlossaryList(app) {
  ...
  const azLetters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').map((l) => {
    ...
    return `<a href="${present ? '#gloss-' + l : '#'}" style="width:28px;height:28px;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:600;border-radius:4px;color:${present ? 'var(--accent)' : 'var(--border)'};text-decoration:none;background:var(--surface);border:1px solid var(--border)">${l}</a>`;
  }).join('');
  const termsHtml = filtered.map((g) => {
    ...
    return `<div id="${first ? 'gloss-' + letter : ''}" style="padding:16px 24px;border-bottom:1px solid var(--border);display:flex;flex-direction:column;gap:4px">
  ...
  return `<div style="display:flex;align-items:center;gap:8px;margin-top:8px">
    ...
    <div style="display:flex;flex-wrap:wrap;gap:8px">${azLetters}</div>
...
export function renderConcepts(app) {
  ...
  return `<div class="dos-page-mid">
    <div><h1 class="dos-h1">Search</h1><p style="color:var(--ink2);font-size:15px;margin-top:8px">Every chapter, concept, and glossary term in one place — searchable, A–Z, and browsable day by day. Terms with a Chapters topic link straight through.</p></div>
    <div style="position:sticky;top:64px;z-index:15;background:var(--bg);padding:4px 0 8px">
      <div style="display:flex;align-items:center;gap:8px;height:52px;padding:0 16px;border:1px solid var(--border);border-radius:8px;background:var(--surface);box-shadow:var(--shadow1)">
