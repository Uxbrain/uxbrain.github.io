...
export function renderSheets(app) {
  ...
  if (!s.sheetId) {
    const cardsHtml = SHEETS.map((sh) => {
      ...
      return `<button data-dos-cardlink data-act="${app.act(() => app.setState({ sheetId: sh.id }))}" style="text-align:left;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:var(--pad-card);cursor:pointer;color:var(--ink);display:flex;flex-direction:column;gap:8px">
    ...
    return `<div class="dos-page">
      <div><h1 class="dos-h1">Cheat Sheets</h1><p style="color:var(--ink2);font-size:15px;margin-top:8px">Book 4 Part E, plus Book 5’s quick-reference layer — everything numeric, tabular, and revision-ready.</p></div>
  ...
  if (cur.kind === 'table') {
    ...
    body = `<div style="display:flex;flex-direction:column;gap:8px">${cur.rows.map((row) => `<div class="dos-card" style="padding:16px 24px">
    ...
    body = `<div style="display:flex;flex-direction:column;gap:16px">
      <div style="background:var(--accent-soft);border:1px solid var(--accent-line);border-radius:8px;padding:24px">
      ...
      <div>
        ...
        <div style="display:flex;flex-direction:column;gap:8px">${cur.structures.map((row) => `<div class="dos-card" style="padding:16px 24px"><span style="font-weight:600">${esc(row[0])}: </span><span style="color:var(--ink2)">${esc(row[1])}</span></div>`).join('')}</div>
      </div>
  ...
  return `<div style="max-width:900px;margin:0 auto;display:flex;flex-direction:column;gap:24px" id="sheet-print-area">
    <div data-no-print="true" style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:16px">
      <div>
